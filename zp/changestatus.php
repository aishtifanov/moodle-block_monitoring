<?php // $Id: changestatus.php,v 1.3 2012/08/24 11:15:29 shtifanov Exp $
	...
	if ($fid == 0) {
		notice(get_string('changestatusnew','block_monitoring'), "listforms.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid");
	}
	...
	if ($confirm == 1) {
	    ...
	    if ($rec = get_record_sql($strsql))	{
	       ...
	       if (!update_record('monit_rayon_listforms', $rec))	{
				error(get_string('errorinupdatingform','block_monitoring'), "listforms.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid");
		   }
		   redirect("listforms.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid", get_string('succesupdatedata','block_monitoring'), 1);
		}
	}
	...
	notice_yesno($s1, "changestatus.php?rid=$rid&amp;fid=$fid&amp;nm=$nm&amp;yid=$yid&amp;sn=$shortname&amp;status=$status&amp;confirm=1", 
                "listforms.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid");
