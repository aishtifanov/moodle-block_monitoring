<?php // $Id: listforms.php,v 1.6 2012/09/24 11:40:51 shtifanov Exp $
    ...
    if ($action == 'excel') 	{
        ...
        print_excel_header('zp_'.$rid.'_'.$nm.'_all');
		create_excel_workbook();
	    foreach($rkps as $rkp)	{
			// print_excel_form('rkp_prr_ro', $datefrom);
			print_excel_form($rkp, $datefrom, 'zp', $rid, $sid, $yid);
		}
	...
	$strtitle = get_string('primarydata', 'block_monitoring');
	...
	print_tabs_years($yid,  "listforms.php?rid=$rid&amp;sid=$sid&amp;nm=$nm&amp;yid=");
    //print_tabs_years_link("listforms.php?rid=$rid&amp;sid=$sid&amp;nm=$nm&amp;yid=", $rid, $sid, $yid, true);

	// print_tabs_years($yid, "listforms.php?rid=$rid&amp;yid=");

	print_tabs_all_months($nm, "listforms.php?rid=$rid&amp;yid=$yid&amp;nm=");
...
*/	
	// notify('<b>Внимание! 10 декабря в 18-00 доступ к исходным данным системы рейтингования ОУ будет закрыт!</b>'); 
    print_footer();
    ...
    foreach($rkps as $rkp)	{
		...
		if ($currstatus < 4 || ($admin_is  || $region_operator_is))  {       //
           // if ($curryearid == $yid || $admin_is)	{
		 		$links['edit']->url = "htmlforms.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;sn=$rkp&amp;fid=";
...
function get_listnameforms()
{
    global $CFG;
    
    $razdels = get_records_select('monit_razdel', "formid = 2 and shortname like 'zp_%'");
    $nameforms = array();
    foreach ($razdels as $razdel)   {
        $nameforms[] = $razdel->shortname;
    } 
