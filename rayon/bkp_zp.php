<?php // $Id: bkp_zp.php,v 1.3 2008/09/12 08:44:28 Shtifanov Exp $
            ...
            foreach ($fields as $field)  {
            	switch ($field->edizm)	{
            		...
            		case 'proc':
							    ...
							    echo "<td align=right>$field->name</td>";
							    echo "<td align=left> ";

	               		        if ($formslist != '')	{
									$rec = get_record_sql("SELECT Sum($field->name_field) AS sum
															FROM {$CFG->prefix}monit_form_bkp_zp
															WHERE listformid in ($formslist)");
									$avg = number_format($rec->sum/$countsform, 2, ',', '');
									echo "{$avg}&nbsp;" . get_string($field->edizm, 'block_monitoring');
								}	else {
									echo '0';
								}
