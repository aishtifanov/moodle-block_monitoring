<?php // $Id: form.php,v 1.6 2009/02/25 08:23:50 Shtifanov Exp $
...
//					print 'idrzd='.$idrzd."select id from {$CFG->prefix}monit_razdel where name='$rzd->name' and formid=$idform";
					$sql = "CREATE TABLE `{$CFG->prefix}monit_college_table_$idrzd` (".
							'`id` int(10) NOT NULL auto_increment,'.
							'`listformid` int(10) NOT NULL,';
//							print $sql;
				break;
				default:
					if(!hotpot_db_table_exists("monit_college_table_$idrzd"))  {
					...
					}  else  {						redirect("$CFG->wwwroot/blocks/monitoring/form/formcollege.php", get_string('errorimportform', 'block_monitoring'));					}
      			}
			}
...
//print "$sql";
			...
			redirect("$CFG->wwwroot/blocks/monitoring/index.php", get_string('completeimportform', 'block_monitoring'));
		}
		...
		echo '<form method="post" enctype="multipart/form-data" action="formcollege.php">'.
