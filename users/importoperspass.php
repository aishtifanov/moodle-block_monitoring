<?php // $Id: importoperspass.php,v 1.9 2011/02/21 12:52:08 shtifanov Exp $
    ...
    $stroperators = get_string('operators', 'block_monitoring') . " ($level - with password)";
...
//	if (!empty($frm) ) {
		...
		if ($um->preprocess_files()) {
		    ...
		    $required = array( "username" => 1,  "password" => 1,
							    "lastname" => 1, "firstname" => 1, "email" => 1,
            ...
            foreach ($header as $i => $h) {
                ...
                $header[$i] = $h;
                if (!isset($required[$h])) {
                    error(get_string('invalidfieldname', 'error', $h), "$CFG->wwwroot/blocks/monitoring/users/importoperspass.php");
                }
                if (isset($required[$h])) {
                    $required[$h] = 0;
                }
			...
			echo 'login;password;lastname;firstname;email<br>';
  			...
  			for($i=1; $i < $size; $i++)  {
				...
				$userusername = $aemail[0];
				if($olduser = get_record("user", "username", $userusername, '', '', '', '', 'id, username, lastname, firstname'))		{
				...
				if ($level == 'udod' || $level == 'dou')	{
					...
					$schooool = get_record_sql("SELECT id, yearid, uniqueconstcode  FROM {$CFG->prefix}monit_school
							     				WHERE id = {$user->idschool}");
                    if ($schooool->yearid == $curryearid)   {                             
					   $user->idschool = $schooool->uniqueconstcode;
                    } else {
    					// $schooool = get_record_sql("SELECT id, yearid, uniqueconstcode  FROM {$CFG->prefix}monit_school
    					//		     				WHERE uniqueconstcode = {$user->idschool}  AND yearid = $curryearid");
...
/// Print the form
    ...
    echo '<center>';
    echo '<form method="post" enctype="multipart/form-data" action="importoperspass.php">'.
