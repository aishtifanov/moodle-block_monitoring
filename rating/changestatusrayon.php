<?php // $Id: changestatusrayon.php,v 1.1 2012/11/01 12:50:15 shtifanov Exp $
	...
	if ($fid == 0) {
		notice(get_string('changestatusnew','block_monitoring'), "$CFG->wwwroot/blocks/monitoring/rayon/listformsrayon.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid");
	}
	...
	if ($confirm == 1) {
	    ...
	    if ($rec = get_record_sql($strsql))	{
	       ...
	       if (!update_record('monit_rayon_listforms', $rec))	{
				error(get_string('errorinupdatingform','block_monitoring'), "$CFG->wwwroot/blocks/monitoring/rating/listformsrayon.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid");
		   }
		   redirect("$CFG->wwwroot/blocks/monitoring/rating/listformsrayon.php?rid=$rid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid", get_string('succesupdatedata','block_monitoring'), 1);
		}
	}
	...
	notice_yesno($s1, "changestatusrayon.php?rid=$rid&amp;fid=$fid&amp;nm=$nm&amp;yid=$yid&amp;sn=$shortname&amp;status=$status&amp;confirm=1", "listformsrayon.php?yid=$yid&amp;rid=$rid");
