<?PHP // $Id: addschool.php,v 1.30 2011/12/16 05:56:31 shtifanov Exp $
    ...
    if ($mode === "new" || $mode === "add" )	{
         $straddschool = get_string('addschool','block_monitoring');
    }
	else 	{
	     $straddschool = get_string('updateschool','block_monitoring');
	 }
	...
	$breadcrumbs .= " -> <a href=\"{$CFG->wwwroot}/blocks/monitoring/school/schools.php?rid=$rid&amp;sid=$school->uniqueconstcode&amp;yid=$yid\">$strschools</a>";
   ...
   $toprow = array();
   for ($i=1; $i<=5; $i++)	{
	   $toprow[] = new tabobject('schooltab'.$i, $CFG->wwwroot."/blocks/monitoring/school/addschool.php?mode=$mode&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;tab=$i",
   ...
   print_tabs($tabs,'schooltab'.$currtab, $inactive, $activetwo);
    ...
    switch ($mode)	{
    	case 'new': $strsql =  "SELECT *  FROM {$CFG->prefix}monit_school
   								WHERE rayonid = $rid AND isclosing=0 AND yearid=$yid";

				 	if ($allschools = get_records_sql($strsql))	{
    	...
    	break;
	 	case 'add': if ($admin_is || $region_operator_is) {
				 		...
				 		$rec->codeppe = required_param('codeppe');
						...
						$rec->numsession = optional_param('numsession', 1, PARAM_INT);

						if (find_form_school_errors($rec, $err, $currtab) == 0) {
							$rec->timemodified = time();
                            $rec->yearid = $yid;
							if ($idnew = insert_record('monit_school', $rec))	{
								 $rec->id = $idnew;
								 $rec->uniqueconstcode = $idnew;
								 if (update_record('monit_school', $rec))	{
									 echo '<div align=center>';
									 notice(get_string('schooladded','block_monitoring'), "$CFG->wwwroot/blocks/monitoring/school/addschool.php?mode=edit&amp;rid=$rid&amp;sid=$idnew&amp;yid=$yid&amp;tab=2");
								   	 echo '</div>';
	 							 } else  {
							  		error(get_string('errorinaddingschool','block_monitoring') . '(insert)', "$CFG->wwwroot/blocks/monitoring/school/schools.php?rid=$rid&amp;yid=$yid");
							  	 }
							} else  {
						  		error(get_string('errorinaddingschool','block_monitoring') . '(update)', "$CFG->wwwroot/blocks/monitoring/school/schools.php?rid=$rid&amp;yid=$yid");
						  	}
		...
		case 'edit':
					if ($sid > 0) 	{
					    $rec = get_record('monit_school', 'id', $sid);
					}
			    	$mode = 'update';
		break;
		case 'update':
					if ($sid > 0) 	{
						...
						if ($frm = data_submitted())  {
						    ...
						    // echo '<pre>'; print_r($rec);  echo '</pre>'; echo '<hr>';
							if (find_form_school_errors($rec, $err, $currtab, $mode) == 0) {
								...
								if (update_record('monit_school', $rec))	{
									 ...
									 echo '<div align=center>';
									 notice(get_string('schoolupdate','block_monitoring') . '</center>', "$CFG->wwwroot/blocks/monitoring/school/addschool.php?mode=edit&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;tab=$currtab");
									 echo '</div>';
								} else
...
<form name="addform" method="post" action="addschool.php">
...
<table cellpadding="5">
<tr valign="top">
    <td align="right"><b><?php  print_string('code_ppe', 'block_monitoring') ?>:</b></td>
    <td align="left">
		<input type="text" id="codeppe" name="codeppe" size="3" maxlength="3" value="<?php if (isset($rec->codeppe)) p($rec->codeppe) ?>" />
		<?php if (isset($err["codeppe"])) formerr($err["codeppe"]); ?>
    </td>
</tr>

<tr valign="top">
    <td align="right"><b><?php  print_string('code_ou', 'block_monitoring') ?>:</b></td>
    <td align="left">
		<input type="text" id="code" name="code" size="6" maxlength="6" value="<?php if (isset($rec->code)) p($rec->code) ?>" />
		<?php if (isset($err["code"])) formerr($err["code"]); ?>
    </td>
</tr>
<tr valign="top">
    <td align="right"><b><?php  print_string('numberschool', 'block_monitoring') ?>:</b></td>
    <td align="left">
...
<tr valign="top">
    ...
    <td align="left"> <?php    unset($choices);
				    $choices[0] = '-';
				    $records = get_records('monit_school_category', 'visible', 1);
				    if ($records)	{
...
<tr valign="top">
    <td align="right"><b><?php  print_string('isukg', 'block_monitoring') ?>:</b></td>
    <td align="left">
	    <select size="1" name="isukg">
	 	   <option <?php if (isset($rec->isukg) && $rec->isukg == 0) echo 'selected' ?> value="0">-</option>
		   <option <?php if (isset($rec->isukg) && $rec->isukg == 1) echo 'selected' ?> value="1"><?php print_string('yes'); ?></option>
		   <option <?php if (isset($rec->isukg) && $rec->isukg == -1) echo 'selected' ?> value="-1"><?php print_string('no'); ?></option>
		</select>
    </td>
</tr>
    ...
    switch ($currtab)	{
    	case 1:
				...
				if ($mode == 'add')  {
					...
					else if (record_exists('monit_school', 'rayonid', $rec->rayonid, 'yearid', $yid, 'number', $rec->number))  {
					...
					if (empty($rec->number)) {
						...
						$f = get_record('monit_school', 'id', $rec->id);
						if ($f->number != $rec->number)  {
							if (record_exists('monit_school', 'rayonid', $rec->rayonid, 'yearid', $yid, 'number', $rec->number))  {
								$err["number"] = get_string("errornumberexist", "block_monitoring");
							}
						}
					}
		        ...
		        if (empty($rec->code) || $rec->code	== '000000')	{
				    $err["code"] = get_string("missingname");
				} else  if (strlen($rec->code)	< 6)	{
					$err['code'] = get_string('errorincodeou', 'block_monitoring');
				} else if (!is_numeric($rec->code))	{
					$err['code'] = get_string('errorincodeou', 'block_monitoring');
				}

		        if (empty($rec->codeppe) || $rec->codeppe	== '000')	{
				    $err["codeppe"] = get_string("missingname");
				} else  if (strlen($rec->codeppe)	< 3)	{
					$err['codeppe'] = get_string('errorincodeppe', 'block_monitoring');
				} else if (!is_numeric($rec->codeppe))	{
					$err['codeppe'] = get_string('errorincodeppe', 'block_monitoring');
				}
