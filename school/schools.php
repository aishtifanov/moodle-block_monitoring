<?php // $Id: schools.php,v 1.24 2010/08/25 09:19:39 Shtifanov Exp $
...
//    $sid = optional_param('sid', '');
    if ($action == 'excel') {
        print_excel_schools($rid, $yid);
        exit();
	}else if ($action == 'ods'){
        print_ods_schools($rid, $yid);
        exit();		
	}
    ...
    if ($admin_is  || $region_operator_is) {  // || $rayon_operator_is)  {
		...
		listbox_rayons("schools.php?yid=$yid&amp;rid=", $rid);
		echo '</table>';
	...
	print_heading($strschools, "center");

	print_tabs_years($yid, "schools.php?rid=$rid&amp;sid=$sid&amp;yid=");

    $table->head  = array ($numberf, get_string('codeppe', 'block_mou_ege'), $strname, $strheadname, $strphone, $straddress, 'WWW', 'Email', $straction);
    $table->align = array ('center','center', 'left', 'left', 'left', 'left', 'left', 'left', 'center');
    $table->class = 'moutable';

    if ($admin_is  || $region_operator_is || $rayon_operator_is)  {
		// $arr_schools =  get_records('monit_school', 'rayonid', $rayon->id, 'isclosing', false);
		$arr_schools =  get_records_sql("SELECT *  FROM {$CFG->prefix}monit_school
		...
		if ($arr_schools) foreach ($arr_schools as $school) {
			...
			if ($admin_is || $region_operator_is || ($rayon_operator_is == $rayon->id)) 	{
                ...
                if ($curryearid == $yid || $admin_is)	{
					...
					$strlinkupdate .= "<a title=\"$title\" href=\"addschool.php?mode=edit&amp;rid={$rayon->id}&amp;sid={$school->id}&amp;yid=$yid\">";
					$strlinkupdate .= "<img src=\"{$CFG->pixpath}/i/edit.gif\" alt=\"$title\" /></a>&nbsp;";
				}
			
        		if ($context = get_record('context', 'contextlevel', CONTEXT_SCHOOL, 'instanceid', $school->id)) {
	   ...
	   */
			...
			$table->data[] = array ($school->number, $school->codeppe, $schoolname, $hname, $phone, $school->realaddress, $wwwwww, $emaill, $strlinkupdate);
		}

	}  else {
		$school =  get_record('monit_school', 'uniqueconstcode', $sid, 'rayonid', $rayon->id, 'yearid', $yid);
	...
	if ($admin_is || $region_operator_is) 	{
		?>	<table align="center">
			<tr>
			<td>
		  <form name="addspec" method="post" action="<?php echo "addschool.php?mode=new&amp;rid={$rayon->id}&amp;yid=$yid" ?>">
			    <div align="center">
				<input type="submit" name="addschool" value="<?php print_string('addschool','block_monitoring')?>">
			    </div>
		  </form>
		  </td>
			<td>
			<form name="download" method="post" action="<?php echo "schools.php?action=excel&amp;rid={$rayon->id}&amp;yid=$yid" ?>">
			    <div align="center">
			...
			<td>
		  	  <form name="setnumber" method="post" action="<?php echo "setnumbers.php?rid={$rayon->id}&amp;yid=$yid" ?>">
			    <div align="center">
				<input type="submit" name="setnumber" value="<?php print_string("setnumber", 'block_monitoring')?>">
			    </div>
			  </form>
			</td>
			
			<td>
			<form name="downloadods" method="post" action="<?php echo "schools.php?action=ods&amp;rid={$rayon->id}&amp;yid=$yid" ?>">
			    <div align="center">
				<input type="submit" name="downloadods" value="<?php print_string('downloadods', 'block_monitoring')?>">
			    </div>
		  </form>
			</td>
    ...
    if ($admin_is  || $region_operator_is || $rayon_operator_is)  {
		if ($arr_schools = get_records_sql("SELECT *  FROM {$CFG->prefix}monit_school
	...
	*/
		   ...
		   */
		        ...
		        $wwwwww = "<a target=\"_blank\" href=\"$school->www\">$school->www</a>";
		        $emaill = "<a href=\"mailto:$school->email\">$school->email</a>";
				$table->data[] = array ($schoolname, $strlinkupdate);
			}
		   	print_color_table($table);
		}
...
function print_excel_schools($rid, $yid)
{
    global $CFG;
    ...
    $exceltable->column = array (4, 30, 20, 15, 30, 30, 20);
 	$exceltable->fields = array('number', 'name', 'fio', 'phones', 'realaddress', 'www', 'email');
 	//$exceltable->fields = array('number'); //  'name', 'fio', 'phones', 'realaddress', 'www', 'email');
...
//	if ($schools =  get_records('monit_school', 'rayonid', $rayon->id, 'number'))	{
...
function print_ods_schools($rid, $yid)
{
    global $CFG;
...
//header('Content-Disposition: attachment; filename=outcomes.ods');


    header("Content-Disposition: attachment; filename=\"{$downloadfilename}.ods\"");
    header("Expires: 0");
    header("Cache-Control: must-revalidate,post-check=0,pre-check=0");
    header("Pragma: public");
