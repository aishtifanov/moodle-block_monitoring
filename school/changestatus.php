<?php // $Id: changestatus.php,v 1.5 2009/02/25 08:23:52 Shtifanov Exp $
	...
	$breadcrumbs .= " -> <a href=\"{$CFG->wwwroot}/blocks/monitoring/school/listforms.php?rid=$rid&amp;sid=$sid\">$school->name</a>";
	$breadcrumbs .= " -> $strformname";
    print_header_mou("$site->shortname: $strformname", $site->fullname, $breadcrumbs);

	if ($fid == 0) {		notice(get_string('changestatusnew','block_monitoring'), "$CFG->wwwroot/blocks/monitoring/school/listforms.php?rid=$rid&amp;sid=$sid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid");	}
	...
	if ($confirm == 1) {
	    ...
	    if ($rec = get_record_sql($strsql))	{
             // print_r($rec);
           $rec->status = $status;
	       if (!update_record('monit_school_listforms', $rec))	{
		   ...
		   redirect("$CFG->wwwroot/blocks/monitoring/school/listforms.php?rid=$rid&amp;sid=$sid&amp;nm=$nm&amp;yid=$yid&amp;fid=$fid", get_string('succesupdatedata','block_monitoring'), 1);
		}
	}
	...
	notice_yesno($s1, "changestatus.php?rid=$rid&amp;sid=$sid&amp;fid=$fid&amp;nm=$nm&amp;yid=$yid&amp;sn=$shortname&amp;status=$status&amp;confirm=1", "listforms.php?rid=$rid&amp;sid=$sid&amp;yid=$yid");
