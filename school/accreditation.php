<?php // $Id: accreditation.php,v 1.19 2009/09/25 07:28:34 Shtifanov Exp $
    ...
    $tab = optional_param('tab', 'schoolreport');
...
*/
	$breadcrumbs .= " -> $straccreditation";
    print_header_mou("$site->shortname: $straccreditation", $site->fullname, $breadcrumbs);
	...
	if ($admin_is  || $region_operator_is  || $rayon_operator_is)	{
		$toprow = array();
	   $toprow[] = new tabobject('rayonreport', "accreditation.php?tab=rayonreport&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid",
 	               get_string('rayonreport', 'block_mou_att'));
	   $toprow[] = new tabobject('schoolreport', "accreditation.php?tab=schoolreport&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid",
 	               get_string('schoolreport', 'block_mou_att'));
	   $tabs = array($toprow);
    ...
    if ($tab == 'rayonreport')	{
	    ...
	    if ($admin_is  || $region_operator_is || $rayon_operator_is)	 {
			...
			listbox_rayons("accreditation.php?tab=$tab&amp;sid=0&amp;yid=$yid&amp;rid=", $rid);
			echo '</table>';
	    }
        ...
        $table->head[] = $strname; 	$table->align[] = 'left';
  		...
  		$criterions =  get_records('monit_accr_criteria');
	    if(!empty($criterions)) {
		...
		foreach ($arr_schools as $school) {
				...
				$strsql = "SELECT Sum({$CFG->prefix}monit_accreditation.mark) AS sum
						   ...
						   WHERE {$CFG->prefix}monit_accreditation.schoolid = {$school->id}";
		        ...
		        $sum += get_olimpiad_mark($school->id);
		        $sum -= get_olimpiad_estimate($school->id);
				...
				$table->data[$i][] = $schoolname;
 		        foreach ($criterions as $criteria) {

					$strsql = "SELECT Sum({$CFG->prefix}monit_accreditation.mark) AS sum
							   FROM {$CFG->prefix}monit_school INNER JOIN {$CFG->prefix}monit_accreditation ON {$CFG->prefix}monit_school.id = {$CFG->prefix}monit_accreditation.schoolid
							   WHERE {$CFG->prefix}monit_accreditation.schoolid = {$school->id} AND criteriaid={$criteria->id}";
    ...
    ///// SCHOOL
		...
		// print_tabs_years($yid, "accreditation.php?tab=$tab&amp;rid=$rid&amp;sid=$sid&amp;yid=");
   		$toprow = array();
...
//		                    if (!$staffview_operator)	{
				...
				if  (($admin_is || $region_operator_is || $rayon_operator_is) && (!$staffview_operator)) {
