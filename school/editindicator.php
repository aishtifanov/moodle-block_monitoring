<?php // $Id: editindicator.php,v 1.11 2009/02/25 08:23:52 Shtifanov Exp $
	...
	$breadcrumbs .= " -> <a href=\"{$CFG->wwwroot}/blocks/monitoring/school/accreditation.php?rid=$rid&amp;sid=$sid\">$straccreditation</a>";
	$breadcrumbs .= " -> <a href=\"{$CFG->wwwroot}/blocks/monitoring/school/indicators.php?rid=$rid&amp;sid=$sid&amp;cid=$cid\">$strindicators</a>";
          ...
          if (!$staffview_operator)	  {
		   ...
		   if ($accr = get_record('monit_accreditation', 'schoolid', $sid, 'criteriaid', $cid, 'indicatorid', $iid))	 {				$accreditation->id = $accr->id;
			    if (!update_record('monit_accreditation', $accreditation))	{
					error(get_string('errorinupdatingindicator','block_monitoring'), "{$CFG->wwwroot}/blocks/monitoring/school/indicators.php?rid=$rid&amp;sid=$sid");
				}
				...
				if ($iid == 17)		{					if ($accr_table = get_record('monit_accr_table_17', 'accrid', $accr->id))  {
					    ...
					    if (!update_record('monit_accr_table_17', $accr_table))	{
							error(get_string('errorinupdatingindicator','block_monitoring') . '(monit_accr_table_17)', "{$CFG->wwwroot}/blocks/monitoring/school/indicators.php?rid=$rid&amp;sid=$sid");
						}
					   ...
					   $accr_table->accrid = $accr->id;
				        ...
				        if (!$new_id = insert_record('monit_accr_table_17', $accr_table))	{
							error(get_string('errorincreatingindicator','block_monitoring') . '(monit_accr_table_17)', "{$CFG->wwwroot}/blocks/monitoring/school/indicators.php?rid=$rid&amp;sid=$sid");
					    }
  					}
				}
		       ...
		       if (!$new_id = insert_record('monit_accreditation', $accreditation))	{
					error(get_string('errorincreatingindicator','block_monitoring'), "{$CFG->wwwroot}/blocks/monitoring/school/indicators.php?rid=$rid&amp;sid=$sid");
			   }
			   if ($iid == 17)	{
				   unset($accr_table);
