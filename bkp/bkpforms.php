...
//    $first = optional_param('first', 0);       // Rayon id
   ...
   */
    ...
    $cerr = 0;
    if ($frm)  {		$cerr=find_form_errors($frm, $err, "monit_bkp_table_$rzd");    }
...
//print_r($table_verify);
			if(!$table_verify){				$listformid = insert_record("monit_school_listforms", $bkp);
				$table_verify = get_record_sql("select id, status from {$CFG->prefix}monit_school_listforms where (schoolid=$sid) and (datemodified=$bkp->datemodified) and (shortname=$rzd)");
...
//print_r($metacolumn);
					if($metacolumn['type'] !='char')  {//print '<br>sdafsdfsadfdsf<br>';						if (isset($err[$metacolumn['name']])){							$bc = 'style="border-color:#FF0000"';
						}  else  {							$bc = '';						}

						if($metacolumn['type'] == 'int')  {
							if($status == 1 && $metacolumn['max_length'] == '15') {								if($s==''){ $s='0';}								$table->data[] = array($hide_s.translitfield($metacolumn['name'], 2).$hide_e, '<b>'.$name.'<b>', "<input maxlength='5' $bc type='hidden' name=".$metacolumn['name'].' size="10" value="'.$s.'">');
							} else {								if($metacolumn['max_length'] == '15') {									if($s==''){ $s='0';}									$table->data[] = array($hide_s.translitfield($metacolumn['name'], 2).$hide_e, '<b>'.$name.'<b>', "<input style='background-color:#00FF00' maxlength='5' readonly='true' $bc type='text' name=".$metacolumn['name'].' size="10" value="'.$s.'">');
								} else {									$table->data[] = array($hide_s.translitfield($metacolumn['name'], 2).$hide_e, $name, "<input maxlength='5' $bc type='text' name=".$metacolumn['name'].' size="10" value="'.$s.'">');
								}
							}
...
//					$n++;
				...
				foreach ($table_fields as $table_field) {					if(trim($table_field->calcfunc) != '') {						$calcfunc = $table_field->calcfunc;
						...
						if($pos===false) {						} else {							$pos=strpos($calcfunc, ',');
							...
							if(isset($temp)) {
								...
								foreach ($sqls as $sql) {									$idrazdel[]=$sql->id;								}
								for($i = 1; $i < count($temp); $i++) {									$sql = get_record_sql("select id from {$CFG->prefix}monit_rayon_listforms where shortname=$idrazdel[$i] and rayonid=$rid");
		...
		if(!$redirect) {			redirect("$CFG->wwwroot/blocks/monitoring/bkp/bkpforms.php?rid=$rid&amp;sid=$sid&amp;nm=$nm&amp;yid=$yid&amp;level=$frm[levelmonit]&amp;rzd=$rzd&amp;fid=$fid&amp;reported=$reported", get_string('succesavedata', 'block_monitoring'));
		} else {
